...
The purpose of this assignment is to design and implement a complex object (`Computer`) using the **Builder Design Pattern**. This solves the telescoping constructor anti-pattern, provides a readable Fluent API through method chaining, enforces object consistency via strict single-field and cross-field validation, and applies Clean Code principles[cite: 7].

---

## 2. Individual Variant
* **Domain:** Computer Configuration[cite: 7]
...
* **Required Presets:** `Office Workstation`, `Mid-Range Gaming PC`, `High-End Gaming PC`[cite: 7].

---

## 3. Part A - The Design Problem (Initial Implementation)
Before refactoring to the Builder pattern, the initial version of the `Computer` class used a large conventional constructor containing over 10 parameters (4 required, 6 optional, across 3 data types, including a nested `Peripherals` value object)[cite: 7].
...
The solution introduces a static nested `Builder` class inside `Computer`. The `Computer` constructor is made private, making the final Product completely **immutable** (all fields are `final`), ensuring thread safety[cite: 7].

* **Fluent API:** Configuration methods return `this` to allow seamless method chaining[cite: 7].
* **Domain-Specific Naming:** Methods like `.enableLiquidCooling()`, `.powerSupply(750)`, and `.withGpu("RTX 4090")` replace generic setters[cite: 7].

---

## 5. Part C - Validation Challenge
Validation is centralized inside the `Builder.build()` method[cite: 7]. It checks:
1. **Single-Field Rules:**
    * `modelName` must not be blank or null.
    * `ramGb` must be within valid hardware boundaries (e.g., $4 \le \text{RAM} \le 128$).
    * `storageGb` must be positive.
2. **Cross-Field Rules (Individual Constraint):**
    * If `gpu` contains `"RTX 4090"`, then `powerSupplyWatts` must be $\ge 750$ **and** `liquidCooling` must be `true`. If violated, an `IllegalArgumentException` or `IllegalStateException` is thrown[cite: 7].

---

## 6. Part D - Preset Configurations (Director)
A `ComputerDirector` class was implemented to encapsulate the construction steps for standard presets, preventing code duplication[cite: 7]:
...
* **High-End Gaming PC:** Top-tier components, liquid cooling, high wattage PSU, featuring the required console output with `🍌`[cite: 7].

---

## 7. Part E - Clean Code: Before $\rightarrow$ After
Applying principles from *Clean Code, Chapter 3*[cite: 7]:
...
The UML class diagram is saved in the repository at `docs/builder-uml.png`[cite: 7].

| Builder Role | Your Class | Responsibility |
| :--- | :--- | :--- |
| **Product** | `Computer` | The complex immutable object being created; holds final hardware attributes[cite: 7]. |
| **Concrete Builder** | `Computer.Builder` | Provides fluent setter methods and executes single/cross-field validation inside `build()`[cite: 7]. |
| **Director** | `ComputerDirector` | Encapsulates reusable preset construction sequences (Office, Gaming)[cite: 7]. |
| **Component** | `Peripherals` | Value object representing bundled monitors and input devices[cite: 7]. |
| **Client** | `ComputerTest` / `Main` | Initiates the building process and consumes the final product[cite: 7]. |

---

## 10. Part H - Automated Testing & Sample Output
The test suite in `ComputerTest.java` contains **10 comprehensive tests**[cite: 7] covering valid builds, invalid states, boundary cases, the individual constraint, and builder reuse independence. All tests pass successfully (`10 tests passed`)[cite: 5, 7].

### Sample Console Output (High-End Preset):
```text
=== Running Computer Builder Demo ===
Built: Office Workstation with 8GB RAM
Built: Ganing Rig Mid with GPU RTX 4060
